...
async function loadLHASA(){
 const all=[]; let offset=0; const size=2000; const max=20000;
 try{
  while(offset<max){
   const params=new URLSearchParams({
    where:"m_haz_pp_f > 0",
    outFields:"name_0,name_2,m_haz_pp_f,h_haz_pp_f,l_haz_pp_f",
   ...
   const j=await r.json(), batch=j.features||[];
   all.push(...batch);
   document.getElementById("lhasaStatus").textContent=`${all.length.toLocaleString()} hazard areas loaded`;
   if(batch.length<size)break;
   offset+=size;
  }
  lhasaFeatures=all;
  ...
  lhasaFeatures=[];
  document.getElementById("lhasaStatus").textContent="NO DATA — NASA guidance unavailable";
 }
}
...
function eventHTML(e){
 ...
 return `<article class="event-row">
  ...
  <div class="meta">
    ...
    Fatalities: ${e.fatalities==null?"Unknown":Number(e.fatalities)} • Injuries: ${e.injuries==null?"Unknown":Number(e.injuries)}
  </div>
  <div class="meta">
    Exposed: ${e.people_exposed==null?"Unknown":Number(e.people_exposed)} • Evacuated: ${e.evacuated==null?"Unknown":Number(e.evacuated)} • Rescued: ${e.rescued==null?"Unknown":Number(e.rescued)} • Buried/Trapped: ${e.buried==null?"Unknown":Number(e.buried)}
...
function updateRatingSuggestionFromForm(){
 ...
 document.getElementById("ratingSuggestion").textContent=`Current evidence suggests roughly AL-${lo}–AL-${hi}. Missing fields do not need to be filled.`;
}
function prefillNASAEvent(idx){
 ...
 setTimeout(()=>{
   ...
   if(form.elements.fatalities)form.elements.fatalities.value=(p.fatality_count==null||p.fatality_count==="")?"":Number(p.fatality_count);
   if(form.elements.injuries)form.elements.injuries.value=(p.injury_count==null||p.injury_count==="")?"":Number(p.injury_count);
   ...
   if(form.elements.al_rating)form.elements.al_rating.value=`AL-${suggested}`;
   document.getElementById("ratingSuggestion").textContent=`NLS provisional suggestion: AL-${suggested} (reasonable range AL-${lo}–AL-${hi}). Unknown fields are already okay.`;
...
function renderOutlooks(){
 ...
 document.getElementById("autoOutlookSummary").innerHTML=autoOutlooks.length?autoOutlooks.map(autoCardHTML).join(""):(lhasaFeatures.length?`<div class="card"><b>Global outlook loaded automatically.</b><br>${lhasaFeatures.length.toLocaleString()} NASA LHASA hazard polygons are on the map. Search is optional and only adds local Day 1–3/hourly guidance.</div>`:'<div class="card"><b>NO DATA</b><br>Global NASA outlook source is unavailable right now.</div>');
...
function renderDays(){
 ...
 document.getElementById("dayCards").innerHTML=[1,2,3].map((day,i)=>{
  ...
  if(auto){
    const p=Math.max(auto.landslide??0,auto.avalanche??0);
    return `<div class="day-card"><div class="meta">${names[i]}</div><div class="prob">${p}%</div><b>Landslide ${auto.landslide??"NO DATA"}% • Avalanche ${auto.avalanche??"NO DATA"}%</b><small>${esc(auto.label)}</small></div>`;
  }
  if(manual)return `<div class="day-card"><div class="meta">${names[i]}</div><div class="prob">${manual}%</div><b>Owner override</b></div>`;
  if(day===1 && lhasaFeatures.length)return `<div class="day-card"><div class="meta">${names[i]}</div><div class="prob">LIVE</div><b>NASA global polygons</b><small>${lhasaFeatures.length.toLocaleString()} hazard areas loaded automatically</small></div>`;
  return `<div class="day-card"><div class="meta">${names[i]}</div><div class="prob">—</div><b>${day===1?"NO DATA":"Search optional"}</b><small>${day===1?"NASA source unavailable":"Search any area for experimental local guidance; no location permission required."}</small></div>`;
 }).join("");
}
...
function signalFromHour({rain,snow,gust,temp,elev}){
 const vals=[rain,snow,gust,temp,elev];
 if(vals.slice(0,4).some(v=>v==null||Number.isNaN(Number(v))))return {landslide:null,avalanche:null};

 rain=Number(rain); snow=Number(snow); gust=Number(gust); temp=Number(temp); elev=Number(elev||0);

 // Experimental NLS signal, NOT a calibrated probability.
 // It intentionally uses a small non-zero background when valid weather exists,
 // then increases with hourly rain / snowfall / strong gusts.
 let land=1;
 if(rain>=0.2) land+=2;
 if(rain>=1) land+=4;
 if(rain>=2.5) land+=7;
 ...
 if(gust>=55) land+=3;
 if(gust>=75) land+=5;
 land=Math.min(70,Math.max(1,Math.round(land)));

 let avalanche=1;
 if(elev<500 && snow<=0) avalanche=1;
 else{
   if(snow>=0.1) avalanche+=3;
   if(snow>=0.5) avalanche+=5;
   if(snow>=1.5) avalanche+=10;
   if(snow>=3) avalanche+=15;
   if(gust>=45) avalanche+=4;
   if(gust>=65) avalanche+=7;
   if(temp>-2 && temp<3) avalanche+=4;
 }
 avalanche=Math.min(70,Math.max(1,Math.round(avalanche)));
 return {landslide:land,avalanche};
}
function dayRiskFromHourly(hours,elev=0){
 if(!hours.length)return {landslide:null,avalanche:null,rain:null,snow:null,gust:null};
 const valid=hours.filter(h=>h.rain!=null&&h.snow!=null&&h.gust!=null&&h.temp!=null);
 if(!valid.length)return {landslide:null,avalanche:null,rain:null,snow:null,gust:null};
 const rain=valid.reduce((s,h)=>s+Number(h.rain||0),0);
 const snow=valid.reduce((s,h)=>s+Number(h.snow||0),0);
 const gust=Math.max(...valid.map(h=>Number(h.gust||0)));
 const maxLand=Math.max(...valid.map(h=>signalFromHour({...h,elev}).landslide||0));
 const maxAval=Math.max(...valid.map(h=>signalFromHour({...h,elev}).avalanche||0));
 return {landslide:maxLand,avalanche:maxAval,rain:+rain.toFixed(1),snow:+snow.toFixed(1),gust:+gust.toFixed(1)};
}
async function fetchAreaForecast(lat,lon){
 const url=`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&hourly=precipitation,snowfall,temperature_2m,wind_gusts_10m&forecast_days=3&timezone=auto`;
 const r=await fetch(url,{cache:"no-store"});if(!r.ok)throw new Error("Forecast unavailable");
 const w=await r.json(), h=w.hourly||{}, elev=Number(w.elevation||0);
...
async function buildAutoOutlook(lat,lon,label){
 const {hours,elev}=await fetchAreaForecast(lat,lon);
 autoOutlooks=[];
 const byDate={};
 ...
 dates.forEach((date,i)=>{
   const risk=dayRiskFromHourly(byDate[date],elev);
   autoOutlooks.push({day:i+1,lat,lon,label,date,...risk});
 });
 maps.outlook.setView([lat,lon],8);
 renderOutlooks();renderDays();
 return {hours,elev};
}
async function buildHourlyForArea(lat,lon,label){
 document.getElementById("localTitle").textContent=label;
 document.getElementById("probabilityChart").innerHTML='<div class="card">Loading hourly guidance…</div>';
 try{
  const {hours,elev}=await fetchAreaForecast(lat,lon);
  const now=Date.now();
  const next24=hours.filter(h=>new Date(h.time).getTime()>=now-3600000).slice(0,24);
  document.getElementById("probabilityChart").innerHTML=next24.map(h=>{
    const s=signalFromHour({...h,elev});
    const land=s.landslide==null?"NO DATA":`${s.landslide}%`;
    const aval=s.avalanche==null?"NO DATA":`${s.avalanche}%`;
    return `<div class="hour-block"><small>${new Date(h.time).toLocaleTimeString([],{hour:"numeric",minute:"2-digit"})}</small><b>${land}</b><span>Landslide</span><b class="secondary-prob">${aval}</b><span>Avalanche</span></div>`;
  }).join("")||'<div class="card">NO DATA for the next 24 hours.</div>';
  document.getElementById("nearbyEvents").innerHTML=events.filter(e=>JSON.stringify(e).toLowerCase().includes(label.toLowerCase())).slice(0,6).map(e=>`<div class="card"><b>${esc(e.al_rating)}</b><h3>${esc(e.title)}</h3><p>${esc(e.location_name)}</p></div>`).join("")||'<div class="card">No matching rated events loaded.</div>';
 }catch(e){
  document.getElementById("probabilityChart").innerHTML='<div class="card"><b>NO DATA</b><br>Hourly forecast source unavailable.</div>';
 }
}
...
function renderLocal(area="Choose an area"){
 document.getElementById("localTitle").textContent=area;
 document.getElementById("probabilityChart").innerHTML='<div class="card"><b>Search an area for hourly guidance.</b><br>No fake 0% values are shown.</div>';
 document.getElementById("nearbyEvents").innerHTML='<div class="card">Search an area to match nearby rated events.</div>';
}
document.getElementById("outlookPlaceSearch").onclick=async()=>{
 ...
 try{
  ...
  await buildAutoOutlook(g.lat,g.lon,g.name);
 }catch(e){alert(e.message)}
 finally{btn.textContent="View Another Area"}
};
...
document.getElementById("placeSearch").onclick=async()=>{
 const q=document.getElementById("placeInput").value.trim(); if(!q)return;
 try{
   const g=await geocodePlace(q);
   await buildHourlyForArea(g.lat,g.lon,g.name);
 }catch(e){
   document.getElementById("probabilityChart").innerHTML='<div class="card"><b>NO DATA</b><br>Could not load that area.</div>';
 }
};
document.getElementById("useGps").onclick=()=>navigator.geolocation?.getCurrentPosition(async p=>{
 maps.main.setView([p.coords.latitude,p.coords.longitude],9);
 await buildHourlyForArea(p.coords.latitude,p.coords.longitude,"GPS area");
 setPage("local");
});
...
document.getElementById("eventForm").onsubmit=e=>{e.preventDefault();submitTable(e.currentTarget,"events",o=>({...o,
  ...
  fatalities:o.fatalities===""?null:Number(o.fatalities),injuries:o.injuries===""?null:Number(o.injuries),
  people_exposed:o.people_exposed===""?null:Number(o.people_exposed),evacuated:o.evacuated===""?null:Number(o.evacuated),
  rescued:o.rescued===""?null:Number(o.rescued),buried:o.buried===""?null:Number(o.buried),
  source_count:o.source_count===""?null:Number(o.source_count),
...
async function refreshAll(){
 ...
 renderScale();renderEvents();renderAlerts();renderMap();renderOutlooks();renderDays();
 if(document.getElementById("localTitle").textContent==="Choose an area")renderLocal();
 updateStamp();
}
...
window.prefillNASAEvent=prefillNASAEvent;
initMaps();
refreshAll();
realtime();updateAuthUI();setInterval(refreshAll,C.AUTO_REFRESH_MS);
})();