 ...
 if(type!=="avalanche" && al==="all"){
  ...
  html+=sourceRows.map(f=>{
   ...
   return `<article class="event-source-row"><div class="source-icon">NASA<br>COOLR</div><div>
    ...
    <div class="media-links">${p.source_link?`<a target="_blank" rel="noopener" href="${esc(p.source_link)}">Source</a>`:""}${p.photo_link?`<a target="_blank" rel="noopener" href="${esc(p.photo_link)}">Photo</a>`:""}</div>
   </div></article>`;
  }).join("");
 }
...
function renderDays(){
 ...
 document.getElementById("dayCards").innerHTML=[1,2,3].map((day,i)=>{
  ...
  return `<div class="day-card"><div class="meta">${names[i]}</div><div class="prob">${p||0}%</div><b>${auto?"Highest automatic local signal":manual?"Highest owner override":"No local outlook selected"}</b></div>`;
 }).join("");
}
